    ...
    for filename in os.listdir(directory_path):
        if filename.endswith(".out"):
            ...
            with open(file_path, 'r') as file:
                matching_lines = []
                for line in file:
                    if ("performance" in line) or ("Msparsity" in line):
                        insert = True
                        continue
                    if insert:
...
if __name__ == "__main__":
    ...
    with open(f'test/GPU_test_{sys.argv[1]}.csv', 'w') as file:
        file.write("matrix,Msparsity,Mtype,Mpattern,id,n,m,nonZeros,blockSize,Rand,sort,mu,sigma,nflop,nMemAcGL,nMemAcSH,AI_O,AI_A,AI,Iperf,flops,effBandGL,effBandSH,RP,N_STREAM\n")
